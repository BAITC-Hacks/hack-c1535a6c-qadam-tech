"""Шаблонные объяснения «почему этот подрядчик здесь» — детерминированные, на фактах.

Используются всегда: как объяснение по умолчанию и как fallback, если LLM
(app/ai, app/llm.py) недоступна. Цитата из описания (best_quote) также
передаётся в LLM как evidence.
"""
