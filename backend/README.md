...
5. **Объяснение** (`app/explain.py`, опционально LLM через `app/llm.py`): факты (цена vs бюджет и соседи по выдаче, свободная дата, языки, часы)
...
`POST /api/parse` — `{"text": "ведущий на свадьбу в Алматы 15 октября до 1.5 млн"}` →
`{city, event_date, event_type, category, budget, duration, language, source: "llm" | "rules"}`.

## AI

- `app/ai/`: OpenAI (структурированный вывод, `temperature=0`), `parse_search_query` и `explain_contractors`.
- `app/llm.py`: передаёт в LLM evidence (доступность, запас бюджета/часов, совпадение языков, цитата из описания,
  «самый доступный в подборке»), кэширует ответ по запросу и при любой ошибке оставляет шаблон из `app/explain.py`.
- Без `OPENAI_API_KEY` (см. `.env.example`) всё работает на шаблонах и `app/rules_parser.py`.
